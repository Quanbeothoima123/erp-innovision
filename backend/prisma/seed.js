...
async function main() {
  ...
  for (const u of attendanceUsers) {
    for (const day of march2026WorkDays) {
      ...
      const checkIn = new Date(
        `2026-03-${String(day).padStart(2, "0")}T08:05:00`,
      );
      const checkOut = new Date(
        `2026-03-${String(day).padStart(2, "0")}T17:35:00`,
      );
      await prisma.attendanceRecord.create({
        data: {
          ...
          overtimeMinutes: 0,
          status: "PRESENT",
          adjustedByUserId: uHRMgr.id,
        },
      });
    }
  }
  console.log(
    `   → Đã tạo ${march2026WorkDays.length * attendanceUsers.length} attendance records\n`,
  );
    ...
    data: [
      {
        ...
        approverUserId: uSrDev1.id,
        stepType: "MANAGER",
        stepOrder: 1,
        status: "APPROVED",
        comment: "Đồng ý",
        actionAt: new Date("2026-03-11"),
      },
      {
        leaveRequestId: lr1.id,
  ...
  const inv1 = await prisma.invoice.create({
    data: {
      ...
      projectId: proj1.id,
      status: "PAID",
      issuedDate: new Date("2025-01-31"),
  ...
  await prisma.invoiceItem.create({
    data: {
      invoiceId: inv1.id,
      description: "Phí phát triển Module HR (Phase 2.3)",
      quantity: 1,
      unit: "gói",
      unitPrice: 600000000,
      amount: 600000000,
  ...
  const inv2 = await prisma.invoice.create({
    data: {
      ...
      projectId: proj3.id,
      status: "SENT",
      issuedDate: new Date("2025-06-30"),
      dueDate: new Date("2025-07-31"),
      subtotal: 400000000,
      taxAmount: 40000000,
      totalAmount: 440000000,
      paidAmount: 440000000,
      outstandingAmount: 0,
      currency: "VND",
      createdByUserId: uAcc.id,
    ...
    data: [
      {
        ...
        contractId: contract1.id,
        invoiceId: inv1.id,
        paymentCode: "PMT-2025-001",
        amount: 660000000,
        currency: "VND",
        amountInVnd: 660000000,
        paymentDate: new Date("2025-02-20"),
        paymentMethod: "BANK_TRANSFER",
        referenceNumber: "FT202502200001",
        status: "COMPLETED",
        receivedBankName: "Vietcombank",
        receivedAccountNumber: "1019000123456",
        confirmedByUserId: uAcc.id,
        confirmedAt: new Date("2025-02-21"),
      ...
      },
      {
        ...
        contractId: contract3.id,
        invoiceId: inv2.id,
        paymentCode: "PMT-2025-009",
        amount: 440000000,
        currency: "VND",
        amountInVnd: 440000000,
        paymentDate: new Date("2025-07-25"),
        paymentMethod: "BANK_TRANSFER",
        referenceNumber: "NS20250725999",
        status: "COMPLETED",
        receivedBankName: "BIDV",
        receivedAccountNumber: "12010000112233",
        confirmedByUserId: uAcc.id,
        confirmedAt: new Date("2025-07-26"),
