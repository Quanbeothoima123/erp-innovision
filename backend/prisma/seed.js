...
// ============================================================
async function main() {
  console.log("🌱 Bắt đầu seed TechVN v3.1...");
  ...
  console.log("  ✅ Payroll config & Compensations");
  ...
  console.log("   tran.thi.bich@techvn.com         → CTO (MANAGER+EMPLOYEE)");
  console.log("   le.van.cuong@techvn.com          → HR Manager (HR+MANAGER)");
  console.log("   mai.thi.linh@techvn.com          → HR Staff (HR)");
  console.log(
    "   pham.thi.dung@techvn.com         → Sales Manager (SALES+MANAGER)",
  );
  console.log(
    "   hoang.van.em@techvn.com          → Finance Manager (ACCOUNTANT+MANAGER)",
  );
  console.log(
    "   nguyen.minh.giang@techvn.com     → Senior Dev (MANAGER+EMPLOYEE)",
  );
  console.log("   vo.thi.huong@techvn.com          → Developer (EMPLOYEE)");
  console.log(
    "   tran.van.hung@techvn.com         → BA Lead (MANAGER+EMPLOYEE)",
  );
  console.log("   le.thi.lan@techvn.com            → Sales (SALES+EMPLOYEE)");
  console.log(
    "   dinh.van.khoa@techvn.com         → Accountant (ACCOUNTANT+EMPLOYEE)",
  );
}
...
async function seedRoles() {
  const data = [
    ...
    },
    {
      code: "MANAGER",
      name: "Quản lý",
      description: "Quản lý phòng ban/dự án, duyệt nghỉ phép bước 1 và OT",
    },
    {
      code: "EMPLOYEE",
      name: "Nhân viên",
...
async function seedUsers(depts, jobs) {
  ...
  for (const u of usersData) {
    ...
    result[key] = await prisma.user.upsert({
      ...
      update: {},
      create: { ...data, passwordHash: DEFAULT_PASSWORD_HASH },
    });
  }
  return result;
}
...
async function seedUserCompensations(u, sc) {
  // Danh sách compensation — bao gồm u.admin
  const compensations = [
    { user: u.admin, base: 60000000, prob: null },
    { user: u.ceo, base: 60000000, prob: null },
    { user: u.cto, base: 50000000, prob: null },
    { user: u.hr_mgr, base: 25000000, prob: null },
    { user: u.hr_staff, base: 14000000, prob: null },
    { user: u.sales_mgr, base: 28000000, prob: null },
    { user: u.sales1, base: 15000000, prob: null },
    { user: u.sales2, base: 14000000, prob: null },
    { user: u.fin_mgr, base: 30000000, prob: null },
    { user: u.accountant, base: 16000000, prob: null },
    { user: u.senior_dev, base: 35000000, prob: null },
    { user: u.dev1, base: 22000000, prob: null },
    { user: u.dev2, base: 20000000, prob: null },
    { user: u.junior_dev, base: 15000000, prob: 12000000 },
    { user: u.tester, base: 18000000, prob: null },
    { user: u.devops, base: 28000000, prob: null },
    { user: u.ba_lead, base: 25000000, prob: null },
    { user: u.ba1, base: 16000000, prob: null },
    { user: u.mkt_mgr, base: 22000000, prob: null },
    { user: u.probation1, base: 14000000, prob: 11000000 },
    // pending1 chưa ACTIVE nên chưa cấu hình lương
  ];
...
async function seedLeaveBalances(u, lt) {
  ...
  for (const b of balances) {
    const remaining = Math.max(0, b.entitled + b.carried - b.used - b.pending);
...
async function seedLeaveRequests(u, lt) {
  ...
  // ── Đang PENDING chờ Manager (bước 1) ────────────────────
...
async function seedAttendance(u, shifts) {
  ...
  // Các ngày làm việc tháng 3/2025 (bỏ T7, CN)
  const workdays = [];
  for (let day = 3; day <= 28; day++) {
    const date = new Date(2025, 2, day);
    if (date.getDay() !== 0 && date.getDay() !== 6) workdays.push(date);
  }
  ...
  // Dữ liệu tháng 3 là dữ liệu lịch sử được Admin nhập thủ công (bootstrap)
  // → status = MANUAL_ADJUSTED, adjustedByUserId = hr_mgr
  for (const user of devUsers) {
    for (const workDate of workdays) {
      ...
      const totalMin = Math.round((checkOut - checkIn) / 60000) - 60;
      ...
      try {
        await prisma.attendanceRecord.create({
          data: {
            ...
            overtimeApprovedMinutes: otMin,
            isRemoteWork: isWFH,
            // Dữ liệu lịch sử nhập thủ công — KHÔNG qua AttendanceRequest
            status: "MANUAL_ADJUSTED",
            adjustedByUserId: u.hr_mgr.id,
            note: "Dữ liệu bootstrap tháng 3/2025",
          },
        });
      } catch (_) {
        /* skip nếu đã tồn tại */
  ...
  // ── AttendanceRequest PENDING hôm nay ─────────────────────
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const todayCheckin = new Date(today);
  todayCheckin.setHours(8, 12, 0, 0);

  // dev1, dev2, ba1 gửi check-in PENDING (chưa duyệt)
  ...
  // tester gửi check-in và được HR duyệt → tạo AttendanceRecord tương ứng
  try {
    const testerRequest = await prisma.attendanceRequest.create({
      data: {
        userId: u.tester.id,
        shiftId: shift.id,
        requestType: "CHECK_IN",
        requestedAt: new Date(today.getTime() + 8 * 3600000),
        workDate: today,
        status: "APPROVED",
    ...
    // Tạo AttendanceRecord từ request đã duyệt
    await prisma.attendanceRecord.create({
      data: {
        userId: u.tester.id,
        shiftId: shift.id,
        workDate: today,
        checkInAt: new Date(today.getTime() + 8 * 3600000),
        totalWorkMinutes: null,
        lateMinutes: 0,
        isRemoteWork: false,
        status: "PRESENT",
        // Liên kết với AttendanceRequest nguồn
        sourceCheckinRequestId: testerRequest.id,
      },
    });
  } catch (_) {}

  // devops, sales1 gửi check-out PENDING
...
async function seedContracts(clients, u) {
  const data = [
    ...
    },
    {
      ...
      status: "ACTIVE",
      title: "Dịch vụ duy trì hệ thống phần mềm hành chính công",
      totalValue: 3000000000,
      receivedAmount: 2700000000,
      remainingAmount: 300000000,
      startDate: d("2023-07-01"),
      endDate: d("2025-06-30"),
      signedDate: d("2023-06-25"),
      signedByUserId: u.ceo.id,
    },
  ];
  ...
  const result = {};
  for (const c of data) {
    const { key, ...contractData } = c;
    result[key] = await prisma.contract.create({ data: contractData });
  }
  return result;
}
...
async function seedProjects(clients, contracts, u) {
  const data = [
    {
      ...
      projectName: "IoT Smart Warehouse System",
      clientId: clients["KH001"].id,
      contractId: contracts["HD001"].id,
    ...
    },
    {
      ...
      projectName: "Construction ERP Phase 1",
      clientId: clients["KH002"].id,
      contractId: contracts["HD002"].id,
    ...
    },
    {
      ...
      projectName: "Hệ thống Hành chính Công TP.HCM — Maintain",
      clientId: clients["KH003"].id,
      contractId: contracts["HD003"].id,
      ...
      spentAmount: 480000000,
      contractValue: 3000000000,
      invoicedAmount: 2850000000,
      receivedAmount: 2700000000,
    },
  ];
  ...
  const result = {};
  for (const p of data) {
    const { key, ...projectData } = p;
    result[key] = await prisma.project.create({ data: projectData });
  }
  return result;
}
...
async function seedProjectAssignments(projects, u) {
  ...
  const projectMap = {
    PRJ001: projects["PRJ001"],
    PRJ002: projects["PRJ002"],
    PRJ003: projects["PRJ003"],
  };
...
async function seedMilestones(projects, u) {
  const milestones = [
    // PRJ001
    {
      project: projects["PRJ001"],
      name: "Phân tích yêu cầu & thiết kế hệ thống",
      ownerUser: u.ba_lead,
    ...
    },
    {
      ...
      ownerUser: u.senior_dev,
      dueDate: d("2024-07-31"),
      status: "DONE",
    },
    {
      project: projects["PRJ001"],
      name: "Tích hợp IoT Platform",
      ownerUser: u.senior_dev,
    ...
    // PRJ002
    {
      project: projects["PRJ002"],
      name: "Kickoff & Requirement gathering",
      ownerUser: u.ba1,
      dueDate: d("2024-07-15"),
      status: "DONE",
    },
    {
      project: projects["PRJ002"],
      name: "Module Nhân sự & Chấm công",
      ownerUser: u.dev2,
  ...
  for (const m of milestones) {
    await prisma.projectMilestone.create({
      data: {
        ...
        name: m.name,
        ownerUserId: m.ownerUser.id,
        dueDate: m.dueDate,
        status: m.status,
        completedAt: m.status === "DONE" ? m.dueDate : null,
      },
    });
  }
}
  ...
  const expenses = [
    {
      project: projects["PRJ001"],
      category: "SOFTWARE",
      title: "AWS IoT Core subscription",
      amount: 15000000,
      submitter: u.devops,
      approver: u.senior_dev,
      status: "APPROVED",
    },
    {
      project: projects["PRJ001"],
      category: "TRAVEL",
      title: "Chi phí đi lại khảo sát kho KH",
      amount: 5200000,
      submitter: u.senior_dev,
      approver: u.ba_lead,
      status: "APPROVED",
    },
    {
      ...
      title: "Thuê BA freelance giai đoạn 2",
      amount: 30000000,
      submitter: u.ba_lead,
      approver: null,
      status: "PENDING",
    },
    {
      project: projects["PRJ002"],
      category: "HARDWARE",
      title: "Server staging environment",
      amount: 45000000,
      submitter: u.devops,
      approver: u.fin_mgr,
      status: "APPROVED",
    ...
    {
      project: projects["PRJ003"],
      category: "LABOR",
      title: "Phí nhân công tháng 3/2025 (O&M)",
      amount: 120000000,
      submitter: u.devops,
      approver: u.fin_mgr,
      status: "APPROVED",
  ...
  for (const e of expenses) {
    await prisma.projectExpense.create({
      data: {
        ...
        amount: e.amount,
        currency: "VND",
        expenseDate: new Date(today.getFullYear(), today.getMonth() - 1, 15),
        submittedByUserId: e.submitter.id,
        approvedByUserId: e.approver?.id ?? null,
        status: e.status,
...
async function seedInvoices(clients, contracts, projects, u) {
  const invoicesData = [
    ...
    },
    {
      ...
      clientId: clients["KH003"].id,
      contractId: contracts["HD003"].id,
      projectId: projects["PRJ003"].id,
      status: "PAID",
      issuedDate: d("2025-01-01"),
      dueDate: d("2025-02-01"),
      subtotal: 136363636,
      taxAmount: 13636364,
  ...
  for (const inv of invoicesData) {
    const { key, ...invData } = inv;
    const created = await prisma.invoice.create({ data: invData });
    result[key] = created;
  }

  // Invoice items cho một số hóa đơn
  const items = [
    {
      invoiceId: result["INV001"].id,
      items: [
        {
          description: "Phí phát triển giai đoạn 1 — Backend & Database",
          quantity: 1,
          unit: "gói",
          unitPrice: 350000000,
          taxRate: 0.1,
        ...
        {
          description: "Phí phát triển giai đoạn 1 — UI/UX & Frontend",
          quantity: 1,
          unit: "gói",
          unitPrice: 200000000,
          taxRate: 0.1,
    ...
    },
    {
      ...
      items: [
        ...
        {
          description: "Phí tài liệu kỹ thuật",
          quantity: 1,
          unit: "bộ",
          unitPrice: 12727272,
          taxRate: 0.1,
  ...
  for (const inv of items) {
    for (const [order, item] of inv.items.entries()) {
...
function calcPayroll(
  ...
  // Thuế TNCN lũy tiến, giảm trừ bản thân 11tr + 4.4tr × dependants
  const deduction = 11000000 + dependants * 4400000;
  const taxableIncome = Math.max(0, gross - bhxh - bhyt - bhtn - deduction);
  let tax = 0;
...
async function seedPayroll(u, sc) {
  ...
  // Kỳ lương tháng 3/2025 — CALCULATING
  ...
  for (const s of staffPayrolls) {
    ...
    // paymentRef an toàn: dùng userCode nếu có, ngược lại dùng id
    const paymentRef = `VCB-2025-01-${s.user.userCode ?? s.user.id.slice(-6)}`;
...
async function seedNotifications(u) {
  const notifications = [
    ...
    },
    {
      ...
      type: "ATTENDANCE_CHECKIN_REQUEST",
      title: "Yêu cầu check-in mới",
      message: "Bùi Văn Ích gửi yêu cầu check-in lúc 08:05 hôm nay.",
      relatedEntityType: "ATTENDANCE_REQUEST",
      isRead: false,
    },
    {
      ...
      type: "ATTENDANCE_CHECKIN_REQUEST",
      title: "Yêu cầu check-in mới",
      message: "Nguyễn Thị Oanh gửi yêu cầu check-in lúc 09:01 hôm nay.",
      relatedEntityType: "ATTENDANCE_REQUEST",
      isRead: false,
    },
    {
    ...
    // ── Chấm công: kết quả duyệt → nhân viên nhận ────────────
    {
      recipientUserId: u.tester.id,
      senderUserId: u.hr_mgr.id,
      type: "ATTENDANCE_REQUEST_APPROVED",
      title: "Check-in được duyệt",
      message: "Yêu cầu check-in hôm nay của bạn đã được duyệt.",
      relatedEntityType: "ATTENDANCE_REQUEST",
      isRead: false,
    },
    // ── Nghỉ phép: đơn mới → Manager nhận ───────────────────
    {
      ...
      message:
        "Bùi Văn Ích xin nghỉ 3 ngày (07–09/04/2025) — chờ duyệt bước 1.",
    ...
    },
    {
      ...
      message:
        "Nguyễn Thị Oanh xin nghỉ 2 ngày (14–15/04), Manager đã duyệt bước 1.",
      relatedEntityType: "LEAVE_REQUEST",
      isRead: false,
    },
    // ── Nghỉ phép: kết quả → nhân viên nhận ──────────────────
    {
      recipientUserId: u.dev1.id,
      senderUserId: u.hr_mgr.id,
      type: "LEAVE_REQUEST_APPROVED",
      title: "Đơn nghỉ phép được duyệt",
      message: "Đơn nghỉ phép 10–12/02/2025 của bạn đã được phê duyệt.",
      isRead: true,
      readAt: d("2025-02-07"),
    },
    {
      ...
      message:
        "Đơn nghỉ 15–16/03/2025 bị từ chối. Lý do: Đang cao điểm release.",
      isRead: true,
      readAt: d("2025-03-12"),
    },
    // ── OT: đơn mới → Manager/HR nhận (đúng chiều) ───────────
    {
      recipientUserId: u.senior_dev.id,
      senderUserId: u.tester.id,
    ...
    // ── OT: kết quả → nhân viên nhận ─────────────────────────
    {
      recipientUserId: u.senior_dev.id,
      senderUserId: u.cto.id,
      type: "OVERTIME_APPROVED",
      title: "Yêu cầu OT được duyệt",
      message: "OT ngày 14/03 (18:00–21:00) đã được duyệt.",
      isRead: true,
      readAt: d("2025-03-13"),
    },
    // ── Lương ────────────────────────────────────────────────
    {
      ...
      title: "Bảng lương tháng 1/2025",
      message: "Bảng lương 01/2025 đã sẵn sàng. NET: 16.842.000 ₫",
      isRead: true,
      readAt: d("2025-02-05"),
    },
    {
      ...
      type: "PAYSLIP_AVAILABLE",
      title: "Bảng lương tháng 1/2025",
      message: "Bảng lương 01/2025 đã sẵn sàng. Tiền thưởng: 3.000.000 ₫",
      isRead: false,
    },
    // ── Hợp đồng / Hóa đơn ───────────────────────────────────
    {
      ...
      message: "HĐ-2023-005 (Sở TT&TT HCM) hết hạn 30/06/2025 — còn 90 ngày.",
      isRead: false,
    },
    {
      ...
      message: "INV-2025-002 (Sở TT&TT — 150.000.000 ₫) quá hạn từ 15/03.",
      isRead: false,
    },
    {
    ...
    // ── Dự án ────────────────────────────────────────────────
    {
      ...
      message:
        'Milestone "Module Vật tư & Kho" (Construction ERP) đã quá hạn 31/03.',
      isRead: false,
    },
  ];
