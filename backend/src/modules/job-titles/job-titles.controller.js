...
const { toJobTitleDto, toJobTitleDetailDto, toJobTitleOptionDto } = require('./job-titles.mapper');
const { successResponse, noContentResponse, paginatedResponse } = require('../../common/utils/response.util');

async function listJobTitles(req, res, next) {
  try {
    const { jobTitles, pagination } = await service.listJobTitles(req.query);
    return paginatedResponse(res, jobTitles.map(toJobTitleDto), pagination, 'Lấy danh sách chức danh thành công');
  } catch (err) { next(err); }
}
...
async function getJobTitleOptions(req, res, next) {
  try {
    ...
    return successResponse(res, options.map(toJobTitleOptionDto), 'Lấy danh sách chức danh thành công');
  } catch (err) { next(err); }
}
...
async function getJobTitleById(req, res, next) {
  try {
    ...
    return successResponse(res, toJobTitleDetailDto(jt, userCount), 'Lấy thông tin chức danh thành công');
  } catch (err) { next(err); }
}
...
async function createJobTitle(req, res, next) {
  try {
    ...
    return successResponse(res, toJobTitleDto(jt), 'Tạo chức danh thành công', 201);
  } catch (err) { next(err); }
}
...
async function updateJobTitle(req, res, next) {
  try {
    ...
    return successResponse(res, toJobTitleDto(jt), 'Cập nhật chức danh thành công');
  } catch (err) { next(err); }
}
