...
const listProjectsSchema = z.object({
  ...
  ...pagination,
  sortBy: z
    .enum(["startDate", "endDate", "createdAt", "projectName", "spentAmount"])
    .default("createdAt"),
  sortOrder: z.enum(["asc", "desc"]).default("desc"),
});

// Tách base schema (ZodObject) để có thể gọi .partial()
...
// Thêm .refine() sau để dùng cho create
const createProjectSchema = createProjectBaseSchema.refine(
  (d) => !d.startDate || !d.endDate || d.endDate >= d.startDate,
  { message: "Ngày kết thúc phải sau ngày bắt đầu", path: ["endDate"] },
);

// .partial() gọi trên ZodObject (base), rồi mới .refine()
const updateProjectSchema = createProjectBaseSchema
  .partial()
...
const createMilestoneSchema = z.object({
  name: z
    .string({ required_error: "Tên milestone là bắt buộc" })
    .min(2)
    .max(191)
    .trim(),
...
const createExpenseSchema = z.object({
  category: z.enum(EXPENSE_CATEGORIES, {
    required_error: "Danh mục chi phí là bắt buộc",
  }),
  title: z
    .string({ required_error: "Tiêu đề chi phí là bắt buộc" })
    .min(2)
    .max(191)
    .trim(),
  description: z.string().max(3000).optional().nullable(),
