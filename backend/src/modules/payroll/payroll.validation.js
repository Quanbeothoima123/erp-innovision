...
const createPeriodSchema = z.object({
  ...
  workingDaysInPeriod: z.coerce.number().int().min(1).max(31).optional().nullable(),
...
const createCompensationSchema = z.object({
  ...
  standardWorkingDays: z.coerce.number().int().min(1).max(31).optional().nullable(),
...
const updateCompensationSchema = createCompensationSchema.partial().omit({ userId: true });
...
const createSalaryComponentSchema = z.object({
  code: z.string({ required_error: 'Mã thành phần là bắt buộc' }).max(50).trim().toUpperCase(),
  name: z.string({ required_error: 'Tên thành phần là bắt buộc' }).min(2).max(191).trim(),
...
const updateSalaryComponentSchema = createSalaryComponentSchema.partial().omit({ code: true });
...
const assignSalaryComponentSchema = z.object({
  userId: z.string({ required_error: 'User ID là bắt buộc' }),
  salaryComponentId: z.string({ required_error: 'Thành phần lương là bắt buộc' }),
...
const listAdjustmentsSchema = z.object({
  ...
  adjustmentType: z.enum(['BONUS', 'DEDUCTION', 'ADVANCE', 'REIMBURSEMENT']).optional(),
