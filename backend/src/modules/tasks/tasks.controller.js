...
const createTask = async (req, res) => {
  ...
  res
    .status(201)
    .json(success(mapper.mapTask(task), "Task đã được tạo thành công"));
};
...
const addComment = async (req, res) => {
  ...
  res
    .status(201)
    .json(success(mapper.mapComment(comment), "Bình luận đã được thêm"));
};

const getComments = async (req, res) => {
  const comments = await service.getComments(req.params.id, req.user);
  res.json(
    success(comments.map(mapper.mapComment), "Lấy bình luận thành công"),
  );
};
...
const updateComment = async (req, res) => {
  ...
  res.json(
    success(mapper.mapComment(comment), "Cập nhật bình luận thành công"),
  );
};
