...
const createTask = async (req, res) => {
  ...
  res.status(201).json(success(mapper.mapTask(task), 'Task đã được tạo thành công'));
};
...
const getTaskStats = async (req, res) => {
  ...
  res.json(success(mapper.mapTaskStats(stats), 'Lấy thống kê task thành công'));
};

const getTaskById = async (req, res) => {
  const task = await service.getTaskById(req.params.id, req.user);
  res.json(success(mapper.mapTaskDetail(task), 'Lấy chi tiết task thành công'));
};

const updateTask = async (req, res) => {
  const task = await service.updateTask(req.params.id, req.body, req.user);
  res.json(success(mapper.mapTask(task), 'Cập nhật task thành công'));
};

const assignTask = async (req, res) => {
  const task = await service.assignTask(req.params.id, req.body, req.user);
  res.json(success(mapper.mapTask(task), 'Gán task thành công'));
};

const updateTaskStatus = async (req, res) => {
  const task = await service.updateTaskStatus(req.params.id, req.body, req.user);
  res.json(success(mapper.mapTask(task), 'Cập nhật trạng thái thành công'));
};

const completeTask = async (req, res) => {
  const task = await service.completeTask(req.params.id, req.body, req.user);
  res.json(success(mapper.mapTask(task), 'Task đã được hoàn thành'));
};

const cancelTask = async (req, res) => {
  const task = await service.cancelTask(req.params.id, req.user);
  res.json(success(mapper.mapTask(task), 'Task đã được huỷ'));
};

const deleteTask = async (req, res) => {
  await service.deleteTask(req.params.id, req.user);
  res.json(success(null, 'Task đã được xóa'));
};
...
const addComment = async (req, res) => {
  ...
  res.status(201).json(success(mapper.mapComment(comment), 'Bình luận đã được thêm'));
};

const getComments = async (req, res) => {
  const comments = await service.getComments(req.params.id, req.user);
  res.json(success(comments.map(mapper.mapComment), 'Lấy bình luận thành công'));
};

const updateComment = async (req, res) => {
  const comment = await service.updateComment(
    req.params.id, req.params.commentId, req.body, req.user,
  );
  res.json(success(mapper.mapComment(comment), 'Cập nhật bình luận thành công'));
};

const deleteComment = async (req, res) => {
  await service.deleteComment(req.params.id, req.params.commentId, req.user);
  res.json(success(null, 'Bình luận đã được xóa'));
};
