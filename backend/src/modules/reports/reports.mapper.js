...
const _pct = (num, den) => {
  ...
  return d === 0 ? 0 : Math.round((Number(num ?? 0) / d) * 1000) / 10;
};
...
function toHRReportDto(raw, year) {
  ...
  const empMap = Object.fromEntries(
    employmentStats.map((s) => [s.employmentStatus, s._count.id]),
  );
...
function toLeaveReportDto(raw) {
  ...
  return {
    ...
    balanceSummary: {
      ...balanceSummary,
      usageRate: _pct(
        balanceSummary.totalUsed,
        balanceSummary.totalEntitled + balanceSummary.totalCarried,
      ),
    },
    topUsers,
...
function toProjectReportDto(raw) {
  ...
  const msMap = Object.fromEntries(
    milestoneStats.map((m) => [m.status, m._count.id]),
  );
...
function toFinanceReportDto(raw) {
  const { period, payments, invoices, ar, revenueByMethod, revenueByClient } =
    raw;
  ...
  return {
    ...
    invoices: {
      byStatus: invMap,
      totalInvoiced: Object.values(invMap).reduce(
        (s, v) => s + v.totalAmount,
        0,
      ),
      totalOutstanding: Object.values(invMap).reduce(
        (s, v) => s + v.outstanding,
        0,
      ),
    },
