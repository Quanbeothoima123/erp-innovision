...
async function getTypeStats() {
  ...
  const map = {};
  for (const r of raw) {
    if (!map[r.type]) map[r.type] = { type: r.type, read: 0, unread: 0, total: 0 };
...
async function notify(opts) {
  const ids = Array.isArray(opts.recipientIds) ? opts.recipientIds : [opts.recipientIds];
  ...
  return repo.createMany(records).catch(err =>
...
const notifyHelpers = {
  ...
  attendanceCheckInRequest(recipientIds, { requesterName, workDate, senderUserId }) {
    return notify({
      recipientIds, senderUserId,
  ...
  attendanceCheckOutRequest(recipientIds, { requesterName, workDate, senderUserId }) {
    return notify({
      recipientIds, senderUserId,
  ...
  attendanceApproved(recipientId, { workDate, senderUserId }) {
    return notify({
      recipientIds: recipientId, senderUserId,
  ...
  leaveRequestCreated(recipientIds, { requesterName, leaveType, startDate, endDate, totalDays, senderUserId, entityId }) {
    return notify({
      recipientIds, senderUserId,
  ...
  leaveApproved(recipientId, { leaveType, startDate, endDate, totalDays, senderUserId }) {
    return notify({
      recipientIds: recipientId, senderUserId,
  ...
  overtimeRequestCreated(recipientIds, { requesterName, workDate, hours, senderUserId, entityId }) {
    return notify({
      recipientIds, senderUserId,
  ...
  projectAssigned(recipientId, { projectName, roleName, senderUserId, entityId }) {
    return notify({
      recipientIds: recipientId, senderUserId,
  ...
  projectStatusChanged(recipientIds, { projectName, newStatus, senderUserId, entityId }) {
    const statusLabels = {
      ACTIVE: 'Đang thực hiện', COMPLETED: 'Hoàn thành',
      ON_HOLD: 'Tạm dừng', CANCELLED: 'Đã hủy',
    };
    return notify({
      recipientIds, senderUserId,
  ...
  milestoneDueSoon(recipientId, { milestoneName, projectName, daysLeft, entityId }) {
    return notify({
      recipientIds: recipientId,
  ...
  contractSigned(recipientIds, { contractCode, clientName, senderUserId, entityId }) {
    return notify({
      recipientIds, senderUserId,
  ...
  contractExpiringSoon(recipientIds, { contractCode, clientName, daysLeft, entityId }) {
    return notify({
      recipientIds,
  ...
  invoiceSent(recipientIds, { invoiceCode, clientName, amount, senderUserId, entityId }) {
    return notify({
      recipientIds, senderUserId,
      type:              'INVOICE_SENT',
  ...
  paymentReceived(recipientIds, { invoiceCode, amount, clientName, senderUserId }) {
    return notify({
      recipientIds, senderUserId,
  ...
  invoiceOverdue(recipientIds, { invoiceCode, clientName, daysOverdue, entityId }) {
    return notify({
      recipientIds,
