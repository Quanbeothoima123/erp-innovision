...
import {
  Check, X, ChevronLeft, ChevronRight, LogIn, LogOut,
  CalendarDays, ListChecks, Plus, Loader2, Clock, AlertCircle,
} from 'lucide-react';
import { toast } from 'sonner';
import * as attendanceService from '../../lib/services/attendance.service';
import type {
  ApiAttendanceRequest, ApiAttendanceRecord, ApiWorkShift, MonthlyStats,
...
function MiniStat({ label, value, color }: { label: string; value: number; color: string }) {
  const c: Record<string, string> = {
    ...
    orange: 'bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-900/20',
    yellow: 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/20',
    purple: 'bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-900/20',
  };
  return (
...
export function MyAttendancePage() {
  ...
  const fetchMyData = useCallback(async () => {
    ...
    try {
      ...
      if (USE_API) {
        ...
        setMyRecords(mockRecords.filter(r => r.userId === uid && (() => {
  ...
  const sendRequest = async (type: 'CHECK_IN' | 'CHECK_OUT') => {
    ...
    try {
      if (USE_API) {
        ...
        const newReq = {
          id: `ar-${Date.now()}`, userId: uid, type, status: 'PENDING' as const,
          requestedTime: now.toISOString(), reason: note || null,
          reviewedAt: null, reviewNote: null, reviewedBy: null,
  ...
  const stats = useMemo(() => {
    if (USE_API && monthStats) {
      return { present: monthStats.presentDays, absent: monthStats.absentDays, leave: monthStats.leaveDays, adjusted: 0, totalLate: monthStats.totalLateMinutes };
    }
  ...
  const calendarData = useMemo(() => {
    ...
    const hDates = new Set((mockHolidays as { date: string; name: string }[]).filter(h => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push({ day: d, dateStr, isWeekend, isHoliday, record, isFuture: dateStr > TODAY });
    }
    return { days, offset };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex border-b border-border gap-1 overflow-x-auto">
        {[
          { key: 'checkin', label: 'Check-in / Check-out', icon: <Clock size={14} /> },
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
          <div>
            ...
            <div className="flex gap-2 mt-2 flex-wrap">
              {hasCheckin && <span className="text-[11px] px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">✓ Đã gửi check-in</span>}
              {hasCheckout && <span className="text-[11px] px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">✓ Đã gửi check-out</span>}
            </div>
          </div>
              ...
              {todayReqs.map(r => {
                ...
                return (
                  <div key={r.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${rType === 'CHECK_IN' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600'}`}>
                      {rType === 'CHECK_IN' ? <LogIn size={14} /> : <LogOut size={14} />}
                    </div>
                    <div className="flex-1">
                      <div className="text-[13px]">{rType === 'CHECK_IN' ? 'Check-in' : 'Check-out'} — {fmtTime(rTime)}</div>
          ...
          {shifts.length > 0 && (
            <div>
              <label className="text-[11px] text-muted-foreground block mb-1">Ca làm việc</label>
          ...
          <div>
            <label className="text-[11px] text-muted-foreground block mb-1">Ghi chú (tuỳ chọn)</label>
            <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="VD: Làm việc remote..."
          ...
          <div className="flex gap-3 flex-wrap">
            <button onClick={() => sendRequest('CHECK_IN')} disabled={sending || hasCheckin}
              ...
              {sending ? <Loader2 size={16} className="animate-spin" /> : <LogIn size={18} />}
              Gửi Check-in
            </button>
            <button onClick={() => sendRequest('CHECK_OUT')} disabled={sending || hasCheckout}
              ...
              {sending ? <Loader2 size={16} className="animate-spin" /> : <LogOut size={18} />}
              Gửi Check-out
            </button>
          </div>
        </div>
          ...
          {loadingData ? (
            ...
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              <div className="hidden sm:grid grid-cols-[100px_120px_1fr_100px_100px] gap-3 px-4 py-2.5 border-b border-border bg-muted/30 text-[11px] font-medium text-muted-foreground uppercase tracking-wide">
                <span>Loại</span><span>Thời gian</span><span>Ghi chú</span><span>Ngày</span><span>Trạng thái</span>
              </div>
              <div className="divide-y divide-border">
                {[...myReqs].reverse().map(r => {
                  ...
                  const rTime = getReqTime(r);
                  return (
                    <div key={r.id} className="grid sm:grid-cols-[100px_120px_1fr_100px_100px] gap-3 px-4 py-3 items-center">
                      <div className={`flex items-center gap-1.5 text-[12px] ${rType === 'CHECK_IN' ? 'text-emerald-600' : 'text-blue-600'}`}>
                        {rType === 'CHECK_IN' ? <LogIn size={13} /> : <LogOut size={13} />}
                      ...
                      <div className="text-[11px] text-muted-foreground">{rTime ? new Date(rTime).toLocaleDateString('vi-VN') : '—'}</div>
                      <span className={`text-[11px] px-2 py-0.5 rounded-full ${reqStatusColors[r.status] ?? ''}`}>{reqStatusLabels[r.status] ?? r.status}</span>
                    </div>
                  );
                })}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button onClick={() => { if (calMonth === 0) { setCalMonth(11); setCalYear(y => y - 1); } else setCalMonth(m => m - 1); }} className="p-1.5 rounded-lg border border-border hover:bg-accent transition">
                <ChevronLeft size={14} />
              </button>
              <span className="text-[14px] font-medium">{monthNames[calMonth]} {calYear}</span>
              <button onClick={() => { if (calMonth === 11) { setCalMonth(0); setCalYear(y => y + 1); } else setCalMonth(m => m + 1); }} className="p-1.5 rounded-lg border border-border hover:bg-accent transition">
            ...
            <div className="hidden sm:grid grid-cols-5 gap-2">
              ...
              <MiniStat label="Phút trễ" value={stats.totalLate} color="yellow" />
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            ...
            <div className="grid grid-cols-7 border-b border-border">
              {['T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'CN'].map(d => (
                <div key={d} className="py-2 text-center text-[11px] text-muted-foreground font-medium">{d}</div>
              ))}
            </div>
            {/* Days */}
            <div className="grid grid-cols-7">
              {Array.from({ length: calendarData.offset }).map((_, i) => (
                <div key={`empty-${i}`} className="border-b border-r border-border min-h-[60px]" />
              ))}
              {calendarData.days.map(({ day, dateStr, isWeekend, isHoliday, record, isFuture }) => {
                ...
                return (
                  <div key={dateStr}
                    ...
                    <div className={`text-[12px] font-medium mb-1 w-6 h-6 flex items-center justify-center rounded-full ${isToday ? 'bg-blue-600 text-white' : isWeekend || isHoliday ? 'text-muted-foreground' : ''}`}>
                      {day}
                    </div>
                    {record && (
                      <span className={`text-[9px] px-1 py-0.5 rounded ${attStatusBg[record.status] ?? ''}`}>
                        {attStatusLabels[record.status] ?? record.status}
                      </span>
                    )}
                    {isHoliday && !record && (
                      <span className="text-[9px] px-1 py-0.5 rounded bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400">Lễ</span>
                    )}
                  </div>
                );
              })}
          ...
          {selectedDay && selectedDayRecord && (
            <div className="bg-card border border-border rounded-xl p-4 space-y-3">
              <h3 className="text-[14px] font-medium">Chi tiết ngày {new Date(selectedDay).toLocaleDateString('vi-VN')}</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-[12px]">
                <div><span className="text-muted-foreground block text-[11px]">Trạng thái</span>
                  <span className={`px-2 py-0.5 rounded-full ${attStatusBg[selectedDayRecord.status] ?? ''}`}>{attStatusLabels[selectedDayRecord.status]}</span>
                </div>
                <div><span className="text-muted-foreground block text-[11px]">Check-in</span>{fmtTime(selectedDayRecord.checkInAt)}</div>
                <div><span className="text-muted-foreground block text-[11px]">Check-out</span>{fmtTime(selectedDayRecord.checkOutAt)}</div>
                <div><span className="text-muted-foreground block text-[11px]">Giờ làm</span>{Math.floor((selectedDayRecord.totalWorkMinutes ?? 0) / 60)}h{(selectedDayRecord.totalWorkMinutes ?? 0) % 60}m</div>
                {(selectedDayRecord.lateMinutes ?? 0) > 0 && (
                  <div><span className="text-muted-foreground block text-[11px]">Phút trễ</span><span className="text-amber-600">{selectedDayRecord.lateMinutes} phút</span></div>
                )}
                {selectedDayRecord.note && (
                  <div className="col-span-2"><span className="text-muted-foreground block text-[11px]">Ghi chú</span>{selectedDayRecord.note}</div>
                )}
              </div>
            </div>
          )}
...
export function ShiftsPage() {
  ...
  const fetchShifts = useCallback(async () => {
    ...
    } catch { toast.error('Không tải được ca làm việc'); } finally { setLoading(false); }
  }, []);

  useEffect(() => { fetchShifts(); }, [fetchShifts]);

  const handleSave = async () => {
    if (!form.name || !form.code) { toast.error('Vui lòng nhập tên và mã ca'); return; }
    setSaving(true);
    try {
      if (USE_API) {
        if (editShift) {
        ...
        if (editShift) {
          ...
          setShifts(prev => [...prev, { id: `ws-${Date.now()}`, ...form, description: form.description || null }]);
        }
      }
      toast.success(editShift ? 'Đã cập nhật ca làm việc' : 'Đã tạo ca làm việc');
      setShowForm(false);
    } catch (err) { toast.error(err instanceof ApiError ? err.message : 'Thất bại'); } finally { setSaving(false); }
  };
  ...
  const openEdit = (s: ApiWorkShift) => { setEditShift(s); setForm({ name: s.name, code: s.code, shiftType: s.shiftType, startTime: s.startTime, endTime: s.endTime, breakMinutes: s.breakMinutes, isActive: s.isActive, description: s.description ?? '' }); setShowForm(true); };
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12 gap-2 text-muted-foreground"><Loader2 size={16} className="animate-spin" /><span className="text-[13px]">Đang tải...</span></div>
      ) : (
            ...
            <div key={s.id} className={`bg-card border border-border rounded-xl p-4 ${!s.isActive ? 'opacity-60' : ''}`}>
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <span className="font-mono text-[10px] bg-muted px-1.5 py-0.5 rounded">{s.code}</span>
                </div>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${s.isActive ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-500'}`}>
              ...
              {isAdmin && (
                <div className="flex gap-1 mt-3">
                  <button onClick={() => openEdit(s)} className="p-1.5 rounded-lg border border-border hover:bg-accent transition text-[12px] flex items-center gap-1">
                    ✏️ Sửa
                  </button>
                  <button onClick={() => setDeleteConfirm(s.id)} className="p-1.5 rounded-lg border border-red-200 hover:bg-red-50 text-red-600 transition text-[12px]">
                    🗑
                  </button>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-card border border-border rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="text-[15px] font-medium">{editShift ? 'Sửa ca làm việc' : 'Thêm ca làm việc'}</h3>
              <button onClick={() => setShowForm(false)} className="p-1.5 rounded-lg hover:bg-accent"><X size={15} /></button>
            </div>
            <div className="p-5 space-y-3">
              {[
                { label: 'Tên ca *', key: 'name', type: 'text', placeholder: 'VD: Ca Sáng' },
                { label: 'Mã ca *', key: 'code', type: 'text', placeholder: 'VD: MORNING' },
                { label: 'Giờ bắt đầu *', key: 'startTime', type: 'time', placeholder: '' },
                { label: 'Giờ kết thúc *', key: 'endTime', type: 'time', placeholder: '' },
                { label: 'Nghỉ giữa ca (phút)', key: 'breakMinutes', type: 'number', placeholder: '60' },
              ].map(f => (
                <div key={f.key}>
              ...
              <div>
                <label className="text-[12px] text-muted-foreground block mb-1">Loại ca</label>
                <select value={form.shiftType} onChange={e => setForm(p => ({ ...p, shiftType: e.target.value }))} className="w-full px-3 py-2 rounded-lg border border-border bg-background text-[13px]">
                  {['MORNING', 'AFTERNOON', 'NIGHT', 'FLEXIBLE', 'SPLIT'].map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>
              <div className="flex items-center gap-2">
                <input type="checkbox" id="shiftActive" checked={form.isActive} onChange={e => setForm(p => ({ ...p, isActive: e.target.checked }))} className="accent-blue-600" />
                <label htmlFor="shiftActive" className="text-[13px] cursor-pointer">Kích hoạt</label>
              </div>
            </div>
            <div className="flex justify-end gap-2 p-4 border-t border-border">
              ...
              <button onClick={handleSave} disabled={saving} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-[13px] flex items-center gap-1.5 disabled:opacity-50">
                {saving && <Loader2 size={13} className="animate-spin" />} {editShift ? 'Lưu' : 'Tạo ca'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-card border border-border rounded-2xl shadow-xl w-full max-w-sm p-5 space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setDeleteConfirm(null)} className="px-3 py-2 rounded-lg border border-border text-[13px] hover:bg-accent">Huỷ</button>
              <button onClick={async () => {
                if (!deleteConfirm) return;
                try {
                  if (USE_API) await attendanceService.deleteShift(deleteConfirm);
                  setShifts(prev => prev.filter(s => s.id !== deleteConfirm));
                  toast.success('Đã xoá ca làm việc');
                } catch (err) { toast.error(err instanceof ApiError ? err.message : 'Xoá thất bại'); }
                setDeleteConfirm(null);
              }} className="px-3 py-2 bg-red-600 text-white rounded-lg text-[13px] hover:bg-red-700">Xoá</button>
            </div>
          </div>
        </div>
      )}
...
export function HolidaysPage() {
  const { can } = useAuth();
  const isAdmin = can('ADMIN', 'HR');
  type Holiday = { id: string; name: string; date: string; isRecurring: boolean; description: string | null };
  ...
  const fetchHolidays = useCallback(async () => {
    ...
    } catch { toast.error('Không tải được ngày lễ'); } finally { setLoading(false); }
  }, [year]);

  useEffect(() => { fetchHolidays(); }, [fetchHolidays]);

  const handleSave = async () => {
    if (!form.name || !form.date) { toast.error('Vui lòng nhập đủ thông tin'); return; }
    setSaving(true);
    try {
      if (USE_API) {
        if (editH) {
          const u = await attendanceService.updateHoliday(editH.id, { ...form, description: form.description || null });
          setHolidays(prev => prev.map(h => h.id === editH.id ? u : h));
        } else {
          const c = await attendanceService.createHoliday({ ...form, description: form.description || null });
          setHolidays(prev => [...prev, c]);
        }
      } else {
        if (editH) setHolidays(prev => prev.map(h => h.id === editH.id ? { ...h, ...form, description: form.description || null } : h));
        else setHolidays(prev => [...prev, { id: `hol-${Date.now()}`, ...form, description: form.description || null }]);
      }
  ...
  const openCreate = () => { setEditH(null); setForm({ name: '', date: '', isRecurring: false, description: '' }); setShowForm(true); };
  const openEdit = (h: Holiday) => { setEditH(h); setForm({ name: h.name, date: h.date, isRecurring: h.isRecurring, description: h.description ?? '' }); setShowForm(true); };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {isAdmin && (
            <button onClick={openCreate} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-[13px] flex items-center gap-1.5 hover:bg-blue-700">
              <Plus size={14} /> Thêm
            </button>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="grid grid-cols-[120px_1fr_2fr_80px_80px] gap-3 px-4 py-2.5 border-b border-border bg-muted/30 text-[11px] font-medium text-muted-foreground uppercase tracking-wide">
            <span>Ngày</span><span>Tên ngày lễ</span><span>Mô tả</span><span>Hàng năm</span><span />
          </div>
          <div className="divide-y divide-border">
            {holidays.length === 0 && (
              <div className="text-center py-12 text-muted-foreground text-[13px]">Không có ngày lễ nào trong năm {year}</div>
            )}
            {holidays.sort((a, b) => a.date.localeCompare(b.date)).map(h => (
              <div key={h.id} className="grid grid-cols-[120px_1fr_2fr_80px_80px] gap-3 px-4 py-3 items-center hover:bg-muted/20 transition">
                <span className="text-[12px] font-mono">{new Date(h.date).toLocaleDateString('vi-VN')}</span>
                <span className="text-[13px] font-medium">{h.name}</span>
                <span className="text-[12px] text-muted-foreground">{h.description || '—'}</span>
                <span className="text-[11px]">{h.isRecurring ? '✓ Có' : 'Không'}</span>
                {isAdmin && (
                  <div className="flex gap-1">
                    <button onClick={() => openEdit(h)} className="p-1 rounded hover:bg-accent text-[12px]">✏️</button>
                    <button onClick={async () => {
                      try {
                        if (USE_API) await attendanceService.deleteHoliday(h.id);
                        setHolidays(prev => prev.filter(x => x.id !== h.id));
                        toast.success('Đã xoá ngày lễ');
                      } catch (err) { toast.error(err instanceof ApiError ? err.message : 'Xoá thất bại'); }
                    }} className="p-1 rounded hover:bg-accent text-[12px] text-red-500">🗑</button>
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-card border border-border rounded-2xl shadow-xl w-full max-w-md p-5 space-y-3">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-[15px] font-medium">{editH ? 'Sửa ngày lễ' : 'Thêm ngày lễ'}</h3>
              <button onClick={() => setShowForm(false)} className="p-1.5 rounded-lg hover:bg-accent"><X size={15} /></button>
            </div>
            {[
            ...
            <div className="flex items-center gap-2">
              <input type="checkbox" id="isRecurring" checked={form.isRecurring} onChange={e => setForm(p => ({ ...p, isRecurring: e.target.checked }))} className="accent-blue-600" />
              <label htmlFor="isRecurring" className="text-[13px] cursor-pointer">Lặp lại hàng năm</label>
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <button onClick={() => setShowForm(false)} className="px-4 py-2 rounded-lg border border-border text-[13px] hover:bg-accent">Huỷ</button>
              <button onClick={handleSave} disabled={saving} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-[13px] flex items-center gap-1.5 disabled:opacity-50">
                {saving && <Loader2 size={13} className="animate-spin" />} {editH ? 'Lưu' : 'Thêm'}
              </button>
            </div>
          </div>
        </div>
