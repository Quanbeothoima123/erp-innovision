    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          <div
            className={`w-16 h-16 rounded-2xl mx-auto mb-3 flex items-center justify-center ${isMandatory ? "bg-yellow-500" : "bg-blue-600"} text-white`}
          >
          ...
          {!isMandatory && (
            <p className="text-muted-foreground text-[14px] mt-1">
              Cập nhật mật khẩu bảo mật cho tài khoản
            </p>
          )}
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  disabled={loading}
                  className="w-full px-3 py-2.5 rounded-lg border border-border bg-background text-[14px] pr-10 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
                <button
                  type="button"
              ...
              {newPw && (
                <div className="mt-2 space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-[11px] ${strengthPercent <= 40 ? "text-red-500" : strengthPercent <= 80 ? "text-yellow-500" : "text-green-500"}`}
                    >
                      {strengthLabel}
                    </span>
                  </div>
                  <div className="space-y-0.5">
                    {checks.map((c, i) => (
                      <div
                        key={i}
                        className={`flex items-center gap-1.5 text-[11px] ${c.ok ? "text-green-600" : "text-muted-foreground"}`}
                      >
            ...
            <div>
              ...
              <input
                ...
                disabled={loading}
                className={`w-full px-3 py-2.5 rounded-lg border bg-background text-[14px] disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-blue-500 ${confirm && confirm !== newPw ? "border-red-500" : "border-border"}`}
              />
            ...
            {!isMandatory && (
              <button
                type="button"
                onClick={() => navigate(-1)}
                className="w-full py-2 text-muted-foreground text-[13px] hover:text-foreground transition-colors"
              >
                ← Quay lại
              </button>
            )}
            {isMandatory && (
              <button
                ...
                className="w-full py-2 text-red-500 text-[13px] hover:text-red-600 transition-colors"
              >
                Đăng xuất
              </button>
            )}
